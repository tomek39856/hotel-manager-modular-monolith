...
class CancelReservationUseCaseTest extends ComponentTest {
  ...
  void shouldConfirmGuestArrivalWhenCorrectReservationIdProvided() {
    ...
    // when:
    useCase.execute(roomReservation.getId());
  ...
  void shouldThrowWhenWrongReservationIdProvided() {
    ...
    // expect:
    assertThrows(NotFoundException.class, () -> useCase.execute(wrongReservationId));
  }
}